...
var (
	_ provider.Provider = &vkeProvider{}
)
...
func (p *vkeProvider) Metadata(_ context.Context, _ provider.MetadataRequest, resp *provider.MetadataResponse) {
	resp.TypeName = "vke"
}

func (p *vkeProvider) Schema(_ context.Context, _ provider.SchemaRequest, resp *provider.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "VMind Kubernetes Engine (VKE). Uses an OpenStack Keystone token as X-Auth-Token. " +
			"Credentials can match the Terraform OpenStack provider (auth_url + user/password + tenant).",
		Attributes: map[string]schema.Attribute{
			"endpoint": schema.StringAttribute{
				Description: "VKE API base URL (e.g. https://example.com/api/v1). Same as ENDPOINT in the vke README.",
				Required:    true,
			},
			"auth_url": schema.StringAttribute{
				Description: "OpenStack Identity (Keystone) URL — same as auth_url in the OpenStack provider.",
				Required:    true,
			},
			...
			"tenant_name": schema.StringAttribute{
				Description: "Project (tenant) name — same as tenant_name in the OpenStack provider.",
				Optional:    true,
			},
...
func (p *vkeProvider) Configure(ctx context.Context, req provider.ConfigureRequest, resp *provider.ConfigureResponse) {
...
func (p *vkeProvider) Resources(_ context.Context) []func() resource.Resource {
...
func (p *vkeProvider) DataSources(_ context.Context) []func() datasource.DataSource {
